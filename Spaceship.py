# program template for Spaceship by CDK
...
def draw(canvas):
    ...
    # draw lives
    canvas.draw_text("Lives: " + str(lives), [10, 25], 25, "White", "monospace")
    
    # draw score
    score_width = frame.get_canvas_textwidth("Score: " + str(score), 25, "monospace")
    canvas.draw_text("Score: " + str(score), [(WIDTH - score_width - 10), 25], 25, "White", "monospace")
...
def rock_spawner():
    ...
    a_rock.angle_vel = (.01 * random.randrange(1, 25) * random.choice([(-1), 1]))
...
a_rock = Sprite([WIDTH / 2, HEIGHT / 2], [1, 1], 0, .05, asteroid_image, asteroid_info)
